...
- [x] [T-001] `ComplianceReport.jsx:47` — Cambiar `'Segregación de Deberes (SoD)'` → `'Separación de Funciones'`

---

## Bloque II — GAP-1: SeparationRulesValidator HARD/SOFT severity

- [x] [T-002] `SeparationRulesValidator.jsx` — Agregar props `onProceedAnyway` y lógica HARD/SOFT:
...
- [x] [T-003] `mockInterceptor.js` — Agregar `severity` a la respuesta de `_handleSeparationRules()` y agregar handler para `POST /api/access/separation-rules/validate` con `_handleValidateSeparationRules(body)`

- [x] [T-004] Tests de `SeparationRulesValidator`:
...
- [x] [T-005] `accessGateway.js` — Agregar método `validateGroupAssignment(userId, groupId)` → `POST /api/access/groups/{groupId}/validate-for-user` con `{ user_id: userId }`

- [x] [T-006] `access.js` — Agregar thunk `validateGroupAssignment` (`access/validateGroupAssignment`)

- [x] [T-007] `mockInterceptor.js` — Agregar handler `POST /api/access/groups/{groupId}/validate-for-user` → `_handleValidateGroupAssignment(body)`:
  - Devuelve `{ valid: true, conflicts: [] }` por defecto
  - Si `body.user_id` termina en '9': devuelve 1 conflicto SOFT (para testing)

- [x] [T-008] `AssignGroup.jsx` — Refactorizar a flujo 2 pasos:
...
- [x] [T-009] Tests de `AssignGroup.jsx`:
...
- [x] [T-010] `accessGateway.js` — Agregar `getGroupCascadeImpact(groupId, addFunctionIds)` → `GET /api/access/groups/{groupId}/cascade-impact?add_function_ids={ids}`

- [x] [T-011] `mockInterceptor.js` — Agregar handler `GET /api/access/groups/{groupId}/cascade-impact`:
...
- [x] [T-012] `GroupComposition.jsx` — En `handleConfirmAdd`:
...
- [x] [T-013] Tests de `GroupComposition.jsx` para cascade:
...
- [x] [T-014] `accessGateway.js` — Agregar `grantExceptionalPermission(userId, payload)` → `POST /api/users/{userId}/exceptional-permissions/`

- [x] [T-015] `access.js` — Agregar thunk `grantExceptionalPermission`

- [x] [T-016] `mockInterceptor.js` — Agregar handler `POST /api/users/{id}/exceptional-permissions/` → `_handleGrantExceptionalPermission(url, body)`:
...
- [x] [T-017] `src/pages/permissions/ExceptionalPermission.jsx` — Crear página:
...
- [x] [T-018] `AppRouter.jsx` — Agregar lazy import + ruta `/permissions/exceptional-permission`:
...
- [x] [T-019] Tests de `ExceptionalPermission.jsx`:
  - Render campos
...
- [x] [T-020] `accessGateway.js` — Agregar:
...
- [x] [T-021] `access.js` — Agregar thunks `fetchExceptionalPermissions` y `revokeExceptionalPermission`

- [x] [T-022] `mockInterceptor.js` — Agregar handlers:
...
- [x] [T-023] `src/pages/permissions/RevokeExceptionalPermission.jsx` — Crear página:
...
- [x] [T-024] `AppRouter.jsx` — Agregar lazy import + ruta `/permissions/revoke-exceptional`:
  - permission: `FunctionCatalog.REVOKE_EXCEPTIONAL`

- [x] [T-025] Tests de `RevokeExceptionalPermission.jsx`:
