...
Definir el flujo completo de trabajo desde que se identifica un
requerimiento hasta que se deploya a producción, asegurando calidad,
trazabilidad y cumplimiento de estándares del proyecto IACT.
...
**Procedimientos Relacionados**: - PROCED-ANALIZAR-REQUISITOS-001 -
...
isort, flake8) - [ ] No hay TODOs pendientes críticos

**Procedimientos Relacionados**: - PROCED-TDD-RED-GREEN-REFACTOR-001 -
PROCED-EJECUTAR-TESTS-001 - PROCED-EJECUTAR-LINTERS-001
...
**Procedimientos Relacionados**: - PROCED-CREAR-PULL-REQUEST-001 -
...
**Procedimientos Relacionados**: - PROCED-CODE-REVIEW-001 -
...
**Procedimientos Relacionados**: - PROCED-MERGE-PR-001 -
PROCED-DEPLOY-STAGING-001 - PROCED-SMOKE-TESTS-001
...
**Pre-requisitos**: - Feature validada en staging - Aprobación de
Product Owner - Ventana de deployment disponible - Backup de BD
completado
...
Feature funcionando en producción - [ ] Equipo notificado del deployment
exitoso

**Procedimientos Relacionados**: - PROCED-DEPLOY-PRODUCCION-001 -
PROCED-ROLLBACK-001 (si falla) - PROCED-POST-DEPLOYMENT-MONITORING-001
...
documentada (si aplica)

**Procedimientos Relacionados**: - PROCED-CERRAR-ISSUE-001 -
PROCED-ACTUALIZAR-DOCS-001 - PROCED-REGISTRAR-METRICAS-DORA-001
...
**Diferencias**: - Branch desde: ``main`` (no desde develop) -
Nomenclatura: ``hotfix/ISSUE-123-descripcion`` - Code review: 1 reviewer
(acelerado) - Deploy directo a producción (skip staging) - Merge back a
main Y develop
...
**Diferencias**: - Branch: ``refactor/descripcion`` - Tests deben seguir
pasando (no agregar nuevos) - Code review enfocado en mejora de calidad
...
**Diferencias**: - Branch: ``spike/descripcion`` - Output: Documento de
hallazgos (no código en producción) - No requiere tests exhaustivos - No
requiere deployment
...
**Acción**: 1. Generar migración: ``python manage.py makemigrations`` 2.
Revisar migración generada 3. Probar rollback:
``python manage.py migrate app_name <numero_anterior>`` 4. Incluir
migración en PR 5. Documentar rollback strategy en PR
...
**Acción**: 1. Comunicar en daily standup 2. Dividir feature en
sub-features más pequeñas 3. Merge incremental con feature flags 4.
Mantener sincronizado con main (merge main → feature diario)
...
**En cada etapa**: - ETAPA 1: Analizar y diseñar - ETAPA 2: Implementar
código y tests - ETAPA 3: Crear PR - ETAPA 4: Responder a comentarios de
reviewers - ETAPA 5-7: Monitorear deployment (si on-call)
...
**Responsabilidades**: - Revisar en < 24 horas - Verificar: - Código
legible - Tests adecuados - Cumple estándares - No viola ADRs - Dejar
comentarios constructivos - Aprobar o solicitar cambios
...
**Responsabilidades**: - Validar en staging - Ejecutar smoke tests -
Reportar bugs encontrados - Aprobar para producción
...
**Responsabilidades**: - Configurar pipelines CI/CD - Ejecutar
deployments (si manual) - Monitorear métricas post-deployment - Ejecutar
...
**Responsabilidades**: - Revisar diseño arquitectónico - Aprobar PRs
complejos/críticos - Aprobar ADRs - Resolver disputes en code reviews
...
**Solución**: 1. Revisar logs de deployment 2. Verificar migración
localmente 3. Si migración es problema: - Corregir migración - Crear
nueva PR con fix - Re-deploy 4. Si problema es otro: - Rollback a
versión anterior - Investigar causa raíz - Crear hotfix si necesario
...
**Agenda**: 1. Revisar métricas del mes 2. Qué funcionó bien 3. Qué no
funcionó 4. Acciones de mejora 5. Actualizar este proceso si necesario
...
**Verificar**: - Métricas DORA - Satisfacción del equipo - Bottlenecks
identificados - Nuevas herramientas/técnicas disponibles - Actualizar
proceso según aprendizajes
