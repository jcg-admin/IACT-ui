...
import errorReducer, { setGlobalError, clearGlobalError } from '@redux/slices/error'
...
describe('ApiErrorAlert — no error', () => {
  it('renders nothing when global error is null', () => {
    const store = buildStore()
    const { container } = renderAlert(store)
...
describe('ApiErrorAlert — error display', () => {
  ...
  it('uses danger severity class for 500', () => {
    ...
    const { container } = renderAlert(store)
    expect(container.querySelector('.api-error-alert--danger')).toBeInTheDocument()
  })
  ...
  it('uses warning severity class for 429', () => {
    ...
    const { container } = renderAlert(store)
    expect(container.querySelector('.api-error-alert--warning')).toBeInTheDocument()
  })
...
describe('ApiErrorAlert — persistent errors excluded', () => {
...
describe('ApiErrorAlert — dismiss', () => {
...
describe('ApiErrorAlert — auto-close', () => {
...
describe('ApiErrorAlert — rate-limit countdown', () => {
...
describe('ApiErrorAlert — retry button', () => {
