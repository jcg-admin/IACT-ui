...
- [x] **T-001** Registrar adminReducer, logsReducer, savedFiltersReducer en `store.js`
- [x] **T-002** Wire `errorHandlingMiddleware` + `errorLoggingMiddleware` en `store.js`
- [x] **T-003** Commit Wave 1

---

## Wave 2 — Completar cobertura de status codes

- [x] **T-004** Agregar 11 clases de error a `apiErrors.js`:
...
- [x] **T-005** Actualizar `getErrorClassByStatusCode` con los 11 nuevos códigos
- [x] **T-006** Actualizar `isRetryableError` — agregar 408, 413 con retryAfter
- [x] **T-007** Mejorar middleware: 429 muestra retryAfter al usuario; 511 hace redirect a loginUrl
- [x] **T-008** Commit Wave 2
...
- [x] **T-009** Crear `src/components/feedback/ApiErrorToast.jsx`
...
- [x] **T-010** Crear `src/components/feedback/ServerErrorBanner.jsx`
...
- [x] **T-011** Montar `ApiErrorToast` y `ServerErrorBanner` en `App.jsx` o layout raíz
  - Dentro del `<Provider>`, fuera de las rutas
  - Un solo punto de montaje para todo el árbol

- [x] **T-012** Crear `src/styles/components/_feedback.scss`
...
- [x] **T-014** Crear `src/pages/errors/NotFoundPage.jsx` (reemplaza `NotFound.jsx` y el inline)
...
- [x] **T-015** Crear `src/pages/errors/AccessDeniedPage.jsx` (extrae inline de AppRouter)
  - 403 con ícono `lock`, mensaje explicativo, botón "Volver" + link a soporte

- [x] **T-016** Crear `src/pages/errors/ServerErrorPage.jsx`
  - Genérico para 500 — ícono `error_outline`, "Error interno del servidor", botón reload

- [x] **T-017** Crear `src/pages/errors/ServiceUnavailablePage.jsx`
  - 503 — ícono `build`, "Servicio en mantenimiento", Retry-After si disponible

- [x] **T-018** Crear `src/styles/components/_error-pages.scss`
...
- [x] **T-019** Actualizar `AppRouter.jsx` — usar páginas extraídas, agregar ruta 500 (para testing)

- [x] **T-020** Commit Wave 4
...
- [x] **T-021** Quitar error-banner local de 9 páginas (logs + admin)
...
- [x] **T-022** Agregar thunks + métodos faltantes a reportsSlice/reportsService
- [x] **T-023** Actualizar 6 páginas de reportes a thunks, quitar try/catch + error local
...
- [x] **T-024** Extender errorLoggingMiddleware para llamar auditService.logEvent en errores 4xx/5xx

- [x] **T-025** Commit Wave 5

---

## Wave 6 — Documentación

- [x] **T-026** Crear `docs/guides/http-error-handling.md`
  Documentar:
...
- [x] **T-027** Commit Wave 6 + push final
